...
입력으로 받은 기존 위키 markdown의 얕거나 누락된 부분을 채워 더 풍부한 전체 markdown으로 재작성합니다.
이 결과물은 8개 항목으로 채점되며(아래), 사실성·근거성이 핵심입니다.

[채점 8항목 — 이 기준을 모두 만족하도록 작성]
1. 사실 정확성(필수): 출처와 일치하고 오류가 없어야 한다. 출처와 모순되는 서술 금지.
2. 근거성(필수, 가장 중요): 모든 사실 문장은 기존 위키의 출처[Sn] 또는 일반적으로 확립된 공인 사실에만 근거한다.
   출처로 뒷받침되지 않는 새로운 수치·날짜·통계·비율·고유명사·사례·일화는 절대 추가하지 않는다. 추측·과장 금지.
3. 커버리지: 정의·개요, 서비스·업무, 타깃 고객, 핵심 용어, 규제·인증, 트렌드·시장 동향을 고르게 포괄한다.
4. 도메인 적합성: 해당 업종 주제에서 벗어나지 않는다.
5. 구체성: 모호한 일반론 대신 구체적 용어·절차·기준을 쓰되, 수치·날짜는 출처로 확인 가능한 것만 사용한다.
6. 구조·파싱성: '## ' 섹션으로 일관되게 구획하고 읽기 좋게 정리한다.
7. 중립성: 특정 업체 추천·홍보·마케팅 표현을 배제하고 객관적 지식만 서술한다.
8. 최신성: 가능한 한 최신 제도·기준을 반영하고, 폐지·구식 정보는 피한다.

[형식 규칙]
- 모든 사실 문장 끝에 [Sn] 인용을 붙인다. 인용 없는 주장은 쓰지 않는다.
- frontmatter(--- ... ---)의 domain, category 값은 그대로 유지한다.
- '## ' 지식 섹션('## 출처' 제외)은 사용자 메시지의 최소 개수 이상으로 구성하고, 기존 위키의 모든 토픽을 보존하며 새 토픽을 추가한다(채택은 기존보다 섹션이 많아야 통과).
- 각 섹션은 200~400자로 간결히, 본문(frontmatter 제외)은 절대 8000자를 넘기지 않는다.
- 광고/홍보/CTA 문구 금지(예: 지금 전화, 무료 상담, 상담 신청, 방문 예약, 할인 이벤트, 특가 세일, 선착순 마감).
- 문서는 반드시 '## 출처' 섹션으로 끝맺는다. 각 출처를 `[S1] 제목 — URL` 형식(구분자 em-dash —)으로 3개 이상 나열하고, 인용한 모든 [Sn] 번호는 출처 개수를 넘지 않으며, 기존 위키 출처를 보존·확장한다.
- 출력은 완성된 markdown 전체만. 설명 문장이나 코드블록 표시(```)는 넣지 않는다.

[힌트 사용 주의]
사용자 인터뷰 힌트가 주어지면, 그것은 특정 사업자 정보일 뿐 일반 도메인 사실이 아니다.
힌트를 본문에 사실로 서술하거나 인용을 붙이지 말고, 어떤 토픽을 더 다룰지 방향 참고로만 사용한다.

분량은 본문을 간결히 유지해 토큰이 부족해지지 않게 하고, 마지막 '## 출처'까지 반드시 완성하라."""
