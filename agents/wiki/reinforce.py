...
하이브리드 흐름:
  ...
  3. 채택 = P1 게이트 통과 AND **축소 아님**(섹션·길이가 기존보다 안 줄어듦 — 보강은 '확장'이어야)
     AND P1 점수 ≥ 기존 저장점수의 80%(강화프롬프트로 깐깐히 매긴 점을 보정). 아니면 기존 유지.
  4. 어느 경우든 pending 삭제
...
# 보강은 강화 프롬프트(출처 원문 없이 깐깐)로 채점돼 일반 프롬프트로 매긴 기존 저장점수보다
# 구조적으로 낮게 나온다. 그 보정으로, 보강이 기존 저장점수의 이 비율 이상이면(+비축소+게이트) 채택.
ADOPT_SCORE_RATIO = 0.80
...
def reinforce(pending_key: str, *, llm: BedrockLLM | None = None, index: WikiIndexStore | None = None) -> dict:
    ...
    기존은 재채점하지 않고 DDB 저장점수로 비교하며(원문 없는 재채점이 풍부한 원본을 부당
    저평가하던 #206 퇴행 방지), 보강이 기존보다 섹션/길이가 줄면(축소) 채택하지 않는다.
    """
    ...
    for _ in range(MAX_CAS_RETRY):
        if industry_exists(category, domain):
            ...
            score_existing = float(meta.get("confidence") or 0.0)  # 저장점수(재채점 X, #206)
        ...
        threshold = score_existing * ADOPT_SCORE_RATIO
        if rev_p1.score < threshold:
            _delete_pending(pending_key)
            return _log(_res("kept_existing", False, domain, score=rev_p1.score,
                        reason=f"p1({rev_p1.score:.3f}) < {ADOPT_SCORE_RATIO:.0%}*existing_stored({score_existing:.3f})={threshold:.3f}"))
