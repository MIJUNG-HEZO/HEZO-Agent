...
- 실패/미달(생성실패·검수호출실패·precheck·점수미달): 모두 index.reject(domain)로 재큐잉
  (attempts++, status→pending+백오프, S3 미변경 → 다음 회차 재시도). reject를 빠뜨리면
  도메인이 claim한 crawling 상태에 갇혀 다시 처리되지 않는다(orphan, #169).
...
def generate_and_store(category: str, domain: str, raw_key: str, *, llm=None, index=None) -> dict:
    ...
    if not gen.ok:
        # 생성 실패(잘림 등)도 재큐잉 — 안 그러면 도메인이 crawling에 갇힘(orphan). #169
        attempts = (index or WikiIndexStore()).reject(domain)
        return {"domain": domain, "stage": "generate_failed", "passed": False,
                "reason": gen.reason, "attempts": attempts}
    ...
    if not rev.ok:
        # 검수 호출 실패(LLM 에러 등)도 재큐잉 — crawling 갇힘 방지(orphan). #169
        attempts = (index or WikiIndexStore()).reject(domain)
        return {"domain": domain, "stage": "review_failed", "passed": False,
                "reason": rev.reason, "attempts": attempts}
