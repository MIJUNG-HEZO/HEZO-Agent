...
배치(EventBridge 하루 5번)의 시작점. due-index에서 pending 우선 → 만료된 done →
lease 만료된 crawling(처리 중 죽어 방치된 stale, orphan 복구) 순으로 1개 골라 claim(찜).
...
def handler(event: dict[str, Any], context: Any = None) -> dict:
    ...
    # pending 우선 → 만료된 done → lease 만료된 crawling(처리하다 죽어 방치된 stale).
    # 마지막 폴백이 orphan(crawling 갇힘) 자가복구 경로다. due()는 now=현재 epoch 기본값 사용.
    candidates = (
        index.due("pending", 1)
        or index.due("done", 1)
        or index.due("crawling", 1)
    )
    if not candidates:
        return {"found": False}
    ...
    # claim 실패 = 다른 실행이 먼저 선점(활성 crawling) → 이번 회차 패스
