...
# ── precheck (6룰 + 기존보다 큼) ────────────────────────────────────────────────
def precheck(md: str, original_body_len: int = 0) -> PrecheckResult:
    body = _body(md)
    ...
    checks = {
        ...
        "⑦기존보다큼": len(body) > original_body_len,
    }
    stats = {
...
4. '## ' 지식 섹션은 6~9개로 구성한다('## 출처' 제외). 무한정 늘리지 않는다.
5. 본문(frontmatter 제외) 길이는 3500~6000자로 간결하게 쓴다. 절대 8000자를 넘기지 않는다.
...
def build_enriched_wiki_md(
    *,
    ...
    user_content = (
        f"[기존 위키 markdown]\n{existing_wiki}\n{hint_block}\n"
        "위 위키를 규칙에 맞게 보강한 전체 markdown을 출력하세요."
    )
    try:
        result = inv.invoke(
            ClaudeInvocationInput(
...
def try_submit_p2_supplement(
    *,
    ...
    chk = precheck(enriched, original_body_len=original_body_len)
    logger.info(
