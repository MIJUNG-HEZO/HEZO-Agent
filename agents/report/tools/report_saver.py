"""llm_report.json을 S3에 저장하는 내부 도구"""
...
def save_report(site_id: str, report: dict) -> str:
    """
    llm_report.json → hezo-artifacts/sites/{site_id}/llm_report.json
    반환: s3_key
    """
    site_id = validate_site_id(site_id)
    key = f"sites/{site_id}/llm_report.json"
    write_json(ARTIFACTS_BUCKET, key, report,
               metadata={"site-id": site_id, "saved-by": "hezo-report-agent"})
    logger.info("llm_report.json 저장: s3://hezo-artifacts/%s", key)
    return key
