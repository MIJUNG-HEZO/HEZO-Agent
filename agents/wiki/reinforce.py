...
하이브리드 흐름:
  ...
  3. 채택 = P1 게이트 통과 AND P1 점수 > 기존 저장점수 AND **축소 아님**(섹션·길이가 기존보다
     안 줄어듦 — 보강은 '확장'이어야). 아니면 기존 유지.
  4. 어느 경우든 pending 삭제
...
def _log(res: dict) -> dict:
    """결과를 CloudWatch에 남긴다(왜 채택/탈락인지 — #206 이전엔 깜깜이였음)."""
    logger.info("reinforce result: stage=%s adopted=%s domain=%s score=%s reason=%s",
    ...
    for _ in range(MAX_CAS_RETRY):
        ...
        if rev_p1.score <= score_existing:
            ...
            return _log(_res("kept_existing", False, domain, score=rev_p1.score,
                        reason=f"p1({rev_p1.score:.3f})<=existing_stored({score_existing:.3f})"))
